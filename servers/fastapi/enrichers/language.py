...
class LanguageEnricher(BaseEnricher):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                results = await self._search(
                    session, api_key,
                    f"essential travel phrases pronunciation {destination} local language common words tourists etiquette tipping 2025",
                )

            return {
                "language_info": results,
                "destination": destination,
            }
        except Exception as e:
            logger.error(f"Language enrichment failed: {e}")
            return {}
    ...
    def to_markdown(self, data: dict) -> str:
        ...
        sections = [f"### Language & Phrases — {data.get('destination', '')}"]
        for item in data.get("language_info", []):
            sections.append(f"- {item.get('content', '')[:300]}")

        return "\n".join(sections)

    def to_slide_data(self, data: dict, layout_id: str) -> dict | None:
        if "travel-language-survival" not in layout_id:
            return None
        return None


registry.register(LanguageEnricher())
