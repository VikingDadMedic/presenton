...
class ConnectivityEnricher(BaseEnricher):
    name = "connectivity"
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                results = await self._search(
                    session, api_key,
                    f"tourist SIM card WiFi quality power outlet type voltage {destination} digital nomad 2025",
                )

            return {
                "connectivity_info": results,
                "destination": destination,
            }
        except Exception as e:
            logger.error(f"Connectivity enrichment failed: {e}")
            return {}
    ...
    def to_markdown(self, data: dict) -> str:
        ...
        sections = [f"### Connectivity & Power — {data.get('destination', '')}"]
        for item in data.get("connectivity_info", []):
            sections.append(f"- {item.get('content', '')[:300]}")

        return "\n".join(sections)

    def to_slide_data(self, data: dict, layout_id: str) -> dict | None:
        if "travel-connectivity" not in layout_id:
            return None
        return None


registry.register(ConnectivityEnricher())
