...
import { BarChart3, BriefcaseBusiness, LogOut, Mic2, Palette, Shield } from 'lucide-react'
...
const SettingSideBar = ({ mode, setMode, selectedProvider, setSelectedProvider }: { mode: 'nanobanana' | 'presenton', setMode: (mode: 'nanobanana' | 'presenton') => void, selectedProvider: SettingsSection, setSelectedProvider: (provider: SettingsSection) => void }) => {
    ...
    return (
        <div className='w-full max-w-[230px] h-screen px-3 pt-[22px] bg-muted flex flex-col'>
            <p className='text-xs text-foreground font-medium border-b mt-[3.15rem] border-border pb-3.5'>FILTER BY:</p>
            <div className='mt-6 flex-1'>
                <p className='text-muted-foreground text-xs font-medium pb-2.5'>Select Mode</p>
                <div className='p-0.5 rounded-lg bg-card w-fit border border-border flex items-center justify-center mb-[34px]'>
                    <button className={`px-3 font-display h-[26px] text-[10px] font-medium rounded-md ${mode === 'presenton' ? 'bg-primary/5 text-primary' : 'text-muted-foreground'}`}
                        onClick={() => setMode('presenton')}
                    >Template Based
                    </button>
                    <svg xmlns="http://www.w3.org/2000/svg" className='mx-1' width="2" height="17" viewBox="0 0 2 17" fill="none">
                        <path d="M1 0V16.5" className='stroke-border' strokeWidth="2" />
                    </svg>
                    <div className='relative'>
                        <button
                            type="button"
                            className='px-3 font-display h-[26px] text-[10px] font-medium rounded-md cursor-not-allowed opacity-60 bg-transparent text-muted-foreground'
                            disabled
                        >
                            Image Based
                        </button>
                ...
                <p className='text-muted-foreground text-xs font-medium pb-2.5'>Select Provider</p>
                {mode === 'presenton' && <div className='space-y-2.5'>
                    <button className={`w-full rounded-[6px] px-3 py-4 flex items-center gap-1.5 border ${selectedProvider === 'text-provider' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'}`} onClick={() => setSelectedProvider('text-provider')}>
                        ...
                        <p className='text-foreground text-xs font-medium'>Text Provider</p>
                    </button>
                    <button className={`w-full rounded-[6px] px-3 py-4 flex items-center gap-1.5 border ${selectedProvider === 'image-provider' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'}`} onClick={() => setSelectedProvider('image-provider')}>
                        ...
                        <p className='text-foreground text-xs font-medium'>Image Provider</p>
                    </button>
                </div>}
                {
                    mode === 'nanobanana' && <div>
                        <button className={`w-full rounded-[6px] px-3 py-4 flex items-center gap-1.5 border bg-primary/5 border-primary/20`}>
                            ...
                            <p className='text-foreground text-xs font-medium'>Nanobanana</p>
                        </button>
                    </div>
                }
            </div>

            <div className='border-t border-border py-5 relative z-50'>
                <p className='text-muted-foreground text-xs font-medium pb-2.5'>Other</p>
                <div className='space-y-2.5'>
                    ...
                    >
                        ...
                        <p className='text-foreground text-xs font-medium'>Appearance</p>
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <p className='text-foreground text-xs font-medium'>Narration</p>
                    </button>
                    <button
                        type="button"
                        className={`w-full rounded-[6px] p-3 py-4 flex items-center gap-1.5 border ${selectedProvider === 'narration-usage' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'}`}
                        onClick={() => setSelectedProvider('narration-usage')}
                    >
                        <div className='relative w-6 h-6 rounded-full overflow-hidden border border-border flex items-center justify-center bg-card'>
                            <BarChart3 className='w-3.5 h-3.5 text-primary' />
                        </div>
                        <p className='text-foreground text-xs font-medium'>Narration usage</p>
                    </button>
                    <button
                        type="button"
                        className={`w-full rounded-[6px] p-3 py-4 flex items-center gap-1.5 border ${selectedProvider === 'agent-profile' ? 'bg-primary/5 border-primary/20' : 'bg-card border-border'}`}
                        onClick={() => setSelectedProvider('agent-profile')}
                    >
                        <div className='relative w-6 h-6 rounded-full overflow-hidden border border-border flex items-center justify-center bg-card'>
                            <BriefcaseBusiness className='w-3.5 h-3.5 text-primary' />
                        </div>
                        <p className='text-foreground text-xs font-medium'>Agent profile</p>
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <p className='text-foreground text-xs font-medium'>Usage Analytics</p>
                    </button>
                    <button
                        type="button"
                    ...
                    >
                        ...
                        <p className='text-foreground text-xs font-medium'>Sign out</p>
                    </button>
                </div>
            </div>
        </div>
